...
class FileSystem53
{
  ...
  static const int FILE_SIZE_FIELD = 1; // Size of file size field in bytes. Maximum file
                           // size allowed in this file system is 192.
...
public:
   ...
   */
  int searchDir(int index, const std::string &symbolic_file_name);
   ...
   *    none
   */
  void deleteDir(int index, int start_pos, int len);
   ...
   */
  int create(const std::string &symbolic_file_name);
  ...
  int open(const std::string &symbolic_file_name);
   ...
   * Parameter(s):
   *    index: File index which indicates the file to be read.
   *    pos: New position in the file. If pos is bigger than file size, set pos
   * to file size.
   * Return:
   *    0 for successful write
   *    -1 value for error case "File hasn't been open"
   ...
   *    List the name and size of files in the directory. (We have only one
